...
export function AdminSidebar() {
  ...
  function NavItem({
    ...
    // For dashboard, only match exactly. For other items, match exact or sub-routes
    ...
    const content = (
      <div
        className={cn(
          ...
          paddingLeft,
          isActive && "bg-[#28a2fd]/10 text-[#28a2fd] dark:bg-[#28a2fd]/20"
        )}
        onClick={() => {
          if (hasChildren) {
            toggleExpanded(itemId)
          }
          // Navigation is handled by Link component below
        }}
    ...
    return (
      <div>
        {item.href && !hasChildren ? <Link href={item.href} onClick={handleNavigation}>{content}</Link> : content}
      ...
      >
        <div className="h-full flex flex-col">
            ...
            <div className="h-16 px-3 flex items-center border-b border-gray-200 dark:border-[#1F1F23]">
              ...
              >
                <img
                  src="/abledger-logo.png"
                  alt="AbLedger"
                  width={32}
                  height={32}
                  className="flex-shrink-0"
                />
                <span className="text-lg font-semibold hover:cursor-pointer bg-gradient-to-r text-gray-900 dark:text-white">
                AbLedger
              </span>
            </Link>
            </div>
            ...
            {showText ? (
              ...
              >
                <img
                  ...
                  className="flex-shrink-0"
                />
                <span className="text-lg font-semibold hover:cursor-pointer bg-gradient-to-r text-gray-900 dark:text-white bg-clip-text  transition-opacity duration-200">
                  abLedger
                </span>
              </Link>
            ) : (
